...
| **Case Shipment Method**            | Defines how the PrintVis shipment will be processed. Options: No Sales Order, Create Sales Order Per Shipment, Create Sales Order Per Shipment And Release, Create Sales Order Per Shipment And Post, Create Posted Shipments. <br><br>**Please Note:** The option **Create Posted Shipment (Without Sales Order)** is made to be able to provide PrintVis shipment information to standard BC Add-on apps, that are reading from BC Posted Sales Shipments. It creates a kind of "Dummy Posted Sales Shipment" that cannot be used for standard BC Sales Order processes. The Shipment cannot be invoiced with a sales invoice because there is no sales order created with the required info, or "Undo" this shipment. Invoicing can be only made with PV Invoices that cannot be related to posted sales shipments! **If sales invoices are supposed to be used/posted, use one of the other options that are creating a sales orders!**|
