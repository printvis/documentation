...
| **Colors Front**                 | Enter the number of colors only for machines with a fixed number of printing plates, such as newspaper presses, where blank (blind) plates are required for unused print positions. For these machines, the number of plates is always calculated as Colors Front + Colors Back (e.g. PV formula 10). <BR>**Important:** The color count entered on the job or job item is ignored in the PrintVis standard formulas like formulas 8 or 10 when using this field!                |
| **Colors Back**                  | See **Colors Front**                                                                                                                               |
...
| **Perfection printing**          | Select if the machine is to make Perfection printing.                                                                                                     |
| **Calculate from**               | Select how the program calculates the quantity:<br> - **States no. of copies**: Most common; calculates based on the finished quantity.<br> - **Copies per roll**: Rare; calculates based on the number of paper reels. |
| **More Items pr. imposition**    | Choose how the program places more items on the sheet if the user inserts more items on the same sheet. Options:<br> - **Standard**: Printed sheet is split horizontally.<br> - **Vertical**: Items placed in vertical divisions.<br> - **Successively**: Especially for label printing, prints more variants in the same process. |
| **Runs per Sheet/Web**          | Calculates the number of necessary runs in a printing process based on the machine's color capacity.                                                                                                                                             |
| **Max. format exceeded**         | Defines the machine's response if a sheet is larger than the printing machine's capacity:<br> - **Blank**: No reaction, suggests sheet division.<br> - **Warning**: Gives a warning, splits the sheet.<br> - **Stop**: Gives a warning, and does not allow insertion. |
| **Trimming the sheet/web**      | Choose the principle for cutting paper if it is too large for the machine:<br> - **Equal split**: Splits the sheet down the middle.<br> - **Cut to maxformat**: Cuts to the machine's max format.<br> - **Use the whole sheet/web**: Optimizes the entire inventory sheet. |
...
| **Colors Front / <BR>Colors Back**                    | See sheet fed description  above.                                                                                                                                                                                            |
