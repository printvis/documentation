...
| To Location / To Bin Code            | When these fields are chosen, the to location and bin codes are always set to these values by default.                                                                                                      | From/To fields General Note          | If the from and to fields are completed, the item posting is a “transfer” type, meaning the item will be moved from one location/bin to another. It is not a consumption of raw materials with impact on job costing.|
